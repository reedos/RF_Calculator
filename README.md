...
- **Port Z₀** is the PNA / system reference impedance, not the DUT’s Z<sub>in</sub> or Z<sub>out</sub>.
- **PNA → DUT:** dBm is *available* source power. VOPP is at the DUT only if the DUT is matched to port Z₀.
- **DUT → PNA:** dBm is *delivered* receiver power. VOPP is at the PNA port. DUT Z<sub>out</sub> does not change that conversion.
