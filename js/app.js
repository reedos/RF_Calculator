(function () {
  ...
  function resultLine() {
    ...
    if (r.drive === "se") {
      return `${dir} SE | ${RF.formatDbm(r.dbm)} dBm | ZPNA ${r.zpna} Ω | ZDUT ${r.zdut} Ω | ${RF.formatVoltage(RF.voppOf(r))} pk-pk`;
    }
    return `${dir} DIFF | ${RF.formatDbm(r.dbmPort)} dBm/port | ZPNA ${r.zpna} Ω | ZDUT ${r.zdut} Ω | ${RF.formatVoltage(r.vppDiff)} VOPP`;
  }
