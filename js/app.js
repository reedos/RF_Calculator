(function () {
  ...
  const PATH_COPY = {
    src: {
      ...
      seLineNote: "into DUT",
      diffCap: "PNA drives DUT · complementary 180°",
    ...
    rx: {
      seCap: "DUT drives PNA · voltage at the receiver port",
      seSrcKicker: "Source",
      seSrcTitle: "DUT",
      seSrcSub: "SE output",
      seLoadKicker: "Load",
      ...
      seLineNote: "into PNA",
      diffCap: "DUT drives PNA · complementary 180°",
      diffSrcKicker: "Source",
      diffSrcTitle: "DUT",
      ...
      voppHint: "at PNA port · type to solve dBm"
    }
  };
  ...
  function compute() {
    ...
    if (state.source === "vopp") {
      ...
      state.result = RF.fromVoppPlane(state.vopp, state.zpna, state.zdut, state.drive, state.path);
      ...
      state.result = RF.fromDbmPlane(state.dbm, state.zpna, state.zdut, state.drive, state.path);
  ...
  function render(ok) {
    ...
    Bench.update({ valid: true, lines: [
      ...
      `Direction: ${state.path === 'src' ? 'PNA → DUT; power is available source power' : 'DUT → PNA; power is delivered receiver power'}. All powers below are per port.`,
  ...
  function renderTable() {
    const zp = state.zpna > 0 ? state.zpna : 50;
    const zd = state.zdut > 0 ? state.zdut : 50;
    els.refCaption.textContent = `PNA→DUT, ZPNA ${zp} Ω, ZDUT ${zd} Ω`;
  ...
  function resultLine() {
    const r = state.result;
    if (!r) return "";
    const dir = state.path === "rx" ? "DUT→PNA" : "PNA→DUT";
    if (r.drive === "se") {
      return `${dir} SE | ${RF.formatDbm(r.dbm)} dBm | ZPNA ${RF.formatNumber(r.zpna)} Ω | ZDUT ${RF.formatNumber(r.zdut)} Ω | ${RF.formatVoltage(RF.voppOf(r))} pk-pk`;
    }
    return `${dir} DIFF | ${RF.formatDbm(r.dbmPort)} dBm/port | ZPNA ${RF.formatNumber(r.zpna)} Ω | ZDUT ${RF.formatNumber(r.zdut)} Ω | ${RF.formatVoltage(r.vppDiff)} VOPP`;
  }
